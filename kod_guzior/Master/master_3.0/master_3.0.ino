...
bool SendPingEsp(uint8_t id){
  structMessage msg = {  COMMAND_LED_ON , id};
  for (uint8_t i=0; i < 8; i++) {
      msg.command = 10 + (i % 2);  // ON/OFF
...
<body>
  ...
  <script>
    ...
    function initializeScoreList(mode) {
      ...
      if (mode === "sprawność") {
        for (let i = 1; i <= 10; i++) {
          scoreList.push({ nick: `-----`, score: 999 });
        }
      } else if (mode === "zręczność") {
        for (let i = 1; i <= 10; i++) {
          scoreList.push({ nick: `-----`, score: 0 });
        }
      }
      updateScoreTable();
...
String buildHTMLPage() {
  ...
  <body>
    ...
    <div>
      <button onclick="sendStart()" style="padding: 10px 20px; font-size: 16px;">START</button>
      <button onclick="sendStop()" style="padding: 10px 20px; font-size: 16px;">STOP</button>
      <button onclick="sendScan()" style="padding: 10px 20px; font-size: 16px;">SCAN</button>
    </div>
    <div id="status">Status: Czekam...</div>
    <div id="espCount">Połączone ESP: 0</div>
    ...
    <div id="status_errorBox">
      <h3>Komunikaty błędów:</h3>
      <ul id="status_errorList"></ul>
    </div>

    <script>
      let ws = new WebSocket("ws://adres_IP_ESP:81/ws");
      let espDevices = {};
      ...
      ws.onmessage = (event) => {
        ...
        if (data.type === "status") {
          document.getElementById("status").innerText = `Status: ${data.value}`;
        }

        if (data.type === "device") {
          espDevices[data.id] = data;
          updateTable();
        }

        if (data.type === "status_error") {
          addStatusError(data.message);
        }
      };
      function sendStart() {
        fetch("/start")
          ...
          .catch(err => {
            addStatusError("Błąd przy wysyłaniu START: " + err);
          });
      }
      ...
      function sendStop() {
        fetch("/stop")
          ...
          .catch(err => {
            addStatusError("Błąd przy wysyłaniu STOP: " + err);
          });
      }
      function sendScan() {
        fetch("/scan")
          ...
          .catch(err => {
            addStatusError("Błąd przy wysyłaniu SCAN: " + err);
          });
      }

      function updateTable() {
        let tableBody = document.getElementById("espTable").getElementsByTagName("tbody")[0];
        tableBody.innerHTML = "";

        let count = 0;
        for (let id in espDevices) {
          let row = tableBody.insertRow();
          row.insertCell(0).innerText = id;
          row.insertCell(1).innerText = espDevices[id].mac;
          row.insertCell(2).innerText = espDevices[id].status;
          count++;
        }

        document.getElementById("espCount").innerText = `Połączone ESP: ${count}`;
      }

      function addStatusError(msg) {
        let li = document.createElement("li");
        li.innerText = msg;
        document.getElementById("status_errorList").prepend(li);
      }
    </script>
  </body>
  </html>
